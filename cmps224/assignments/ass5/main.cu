...
void histogram_gpu(unsigned char *image_d, unsigned int *bins_d,
                   unsigned int width, unsigned int height) {
...
void histogram_cpu(unsigned char *image, unsigned int *bins, unsigned int width,
                   unsigned int height) {
...
int main(int argc, char **argv) {
  ...
  // Copy data from GPU
  startTime(&timer);
  cudaMemcpy(bins_gpu, bins_d, NUM_BINS * sizeof(unsigned int),
  ...
  stopTime(&timer);
  printElapsedTime(
      timer, "GPU kernel time (with privatization and shared memory)", GREEN);
  ...
  printElapsedTime(timer,
                   "GPU kernel time (with privatization, shared memory, and "
                   "thread coarsening)",
                   GREEN);
