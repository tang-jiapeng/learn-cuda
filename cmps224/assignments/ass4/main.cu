...
void convolution_gpu(float *input_d, float *output_d, unsigned int width,
                     unsigned int height) {
  ...
  convolution_kernel<<<numBlocks, numThreadsPerBlock>>>(input_d, output_d,
                                                        width, height);
}

void convolution_cpu(float filter[][FILTER_DIM], float *input, float *output,
                     unsigned int width, unsigned int height) {
...
int main(int argc, char **argv) {
  ...
  // Copy data to GPU
  startTime(&timer);
  cudaMemcpy(input_d, input, width * height * sizeof(float),
             cudaMemcpyHostToDevice);
  cudaMemcpyToSymbol(filter_c_, filter,
  ...
  // Copy data from GPU
  startTime(&timer);
  cudaMemcpy(output_gpu, output_d, width * height * sizeof(float),
  ...
  for (unsigned int row = 0; row < height; ++row) {
    for (unsigned int col = 0; col < width; ++col) {
      float diff =
          (output_cpu[row * width + col] - output_gpu[row * width + col]) /
          output_cpu[row * width + col];
      const float tolerance = 0.00001;
      if (diff > tolerance || diff < -tolerance) {
        printf(
            "Mismatch at row %u, col %u (CPU result = %e, GPU result = %e)\n",
            row, col, output_cpu[row * width + col],
            output_gpu[row * width + col]);
        exit(0);
      }
    }
  }
